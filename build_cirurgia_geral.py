...
<header>
  ...
  <button class="h-btn grn" onclick="exportResumoPDF()" style="margin-left:4px">
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="8" y1="9" x2="16" y2="9"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="12" y2="17"/></svg>
    Resumo Diário
  </button>
...
<div class="sec" id="sec-lista">
  ...
  <div style="display:flex;gap:10px;margin-bottom:16px;align-items:center;flex-wrap:wrap">
    ...
    <button class="btn btn-sec btn-sm" onclick="exportListaPDF()">
...
/* ── FORM ── */
function clearForm(){
  ['r-nome','r-idade','r-diag-outro','r-cirurgiao','r-ajud1','r-ajud2',
...
function getJsPDF(){
  ...
  if(!lib){ toast('jsPDF não disponível — verifique a ligação à internet', false); }
  return lib;
}
...
function pdfHeader(doc, titulo){
  ...
  doc.rect(0,0,W,30,'F');
  try{ doc.addImage(LOGO_B64,'JPEG',W-20,3,16,16,'','FAST'); }catch(e){}
  doc.setTextColor(255,255,255);
  doc.setFontSize(12); doc.setFont(undefined,'bold');
  doc.text('Hospital do Prenda',12,11);
  doc.setFontSize(7.5); doc.setFont(undefined,'normal');
  doc.text('GEPE — Departamento de Estatística Médica',12,17);
  doc.setFontSize(9.5); doc.setFont(undefined,'bold');
  doc.text(titulo,12,24);
  return 36;
}
...
function pdfFooter(doc){
  ...
  doc.text('Hospital do Prenda · Cirurgia Geral BU · '+new Date().toLocaleString('pt-PT'),12,H-4);
  doc.text('Pág. '+doc.internal.getNumberOfPages(),W-12,H-4,{align:'right'});
}

function pdfEquipaBox(doc, eq, y){
  var W = doc.internal.pageSize.getWidth();
  var internos = (eq.internos||[]).filter(function(m){ return m.trim(); });
  var estag = (eq.estagiarios||[]).filter(function(m){ return m.trim(); });
  ...
  doc.roundedRect(12,y,W-24,26,2,2,'S');

  doc.setFont(undefined,'bold'); doc.setFontSize(7.5); doc.setTextColor(15,43,74);
  doc.text('EQUIPA DE SERVIÇO',15,y+6);
  doc.setFont(undefined,'normal'); doc.setFontSize(8); doc.setTextColor(0,0,0);

  var espLine = 'Especialista: '+espStr;
  doc.text(espLine,15,y+13);
  doc.text('Internos: '+(internos.length?internos.join(', '):'—'),15,y+19);
  doc.text('Estagiários: '+(estag.length?estag.join(', '):'—'),W/2,y+19);
  return y+30;
}
...
function calcStats(recs){
  ...
  var obt = recs.filter(function(r){ return r.resultado==='obito'; }).length;
  var men = recs.filter(function(r){ return parseInt(r.idade)<15; }).length;
  var cats = {'H≥15':0,'F≥15':0,'H<15':0,'F<15':0};
  recs.forEach(function(r){
    var ag = parseInt(r.idade)<15?'<15':'≥15';
    var k = (r.sexo==='M'?'H':'F')+ag;
    cats[k]=(cats[k]||0)+1;
  });
  var diagMap={};
  recs.forEach(function(r){ if(r.diag) diagMap[r.diag]=(diagMap[r.diag]||0)+1; });
  var procMap={};
  recs.forEach(function(r){ if(r.procCir) procMap[r.procCir]=(procMap[r.procCir]||0)+1; });
  return {total:total,urg:urg,ele:ele,comp:comp,obt:obt,men:men,mai:total-men,cats:cats,diagMap:diagMap,procMap:procMap};
}

/* ── PDF REUNIÃO MATINAL ── */
function exportPDF(){
  var jsPDFLib = getJsPDF(); if(!jsPDFLib) return;
  var doc = new jsPDFLib({orientation:'portrait',unit:'mm',format:'a4'});
  var W = doc.internal.pageSize.getWidth();
  var today = todayStr();
  var recs = state.registos.filter(function(r){ return r.data===today; });
  var eq = state.equipa;
  var st = calcStats(recs);

  var y = pdfHeader(doc,'Cirurgia Geral — Relatório Reunião Matinal');
  doc.setTextColor(0,0,0);

  // Data e turno
  doc.setFontSize(8.5); doc.setFont(undefined,'normal');
  doc.text('Data: '+fmtDate(today)+'  |  Turno: '+getLabelTurno(eq.turno),12,y); y+=6;

  // Equipa
  y = pdfEquipaBox(doc,eq,y);

  // Resumo KPIs
  doc.autoTable({
    startY:y,
    head:[['Total','Urgentes','Eletivas/Semi','Complicações','Óbitos','≥15 Anos','<15 Anos']],
    body:[[st.total,st.urg,st.ele,st.comp,st.obt,st.mai,st.men].map(String)],
    theme:'grid',
    headStyles:{fillColor:[15,43,74],textColor:255,fontSize:7,fontStyle:'bold',halign:'center'},
  ...
  // Demografias
  doc.setFont(undefined,'bold'); doc.setFontSize(8); doc.setTextColor(15,43,74);
  doc.text('DEMOGRAFIAS',12,y); y+=4; doc.setTextColor(0,0,0);
  doc.autoTable({
    startY:y,
    head:[['','H ≥ 15','F ≥ 15','H < 15','F < 15','Total']],
    body:[['Intervenções',st.cats['H≥15'],st.cats['F≥15'],st.cats['H<15'],st.cats['F<15'],st.total]],
    theme:'grid',
    headStyles:{fillColor:[15,43,74],textColor:255,fontSize:7.5,halign:'center'},
  ...
  if(recs.length){
    doc.setFont(undefined,'bold'); doc.setFontSize(8); doc.setTextColor(15,43,74);
    doc.text('INTERVENÇÕES CIRÚRGICAS ('+recs.length+')',12,y); y+=4; doc.setTextColor(0,0,0);
    var rows = recs.map(function(r,i){
      return [
        (i+1)+'.',
        r.hora||'—',
        (r.sexo==='M'?'H':'F')+(r.idade?' '+r.idade+'a':''),
        r.diag||'—',
        r.procCir||'—',
        r.tipo==='urgente'?'Urgente':r.tipo==='eletiva'?'Eletiva':'Semiurg.',
        r.resultado==='ok'?'OK':r.resultado==='complicacao'?'Complicação':'Óbito',
        r.destino||'—'
      ];
    });
    doc.autoTable({
      startY:y,
      head:[['#','Hora','Doente','Diagnóstico','Procedimento','Tipo','Resultado','Destino']],
      body:rows, theme:'striped',
      headStyles:{fillColor:[15,43,74],textColor:255,fontSize:6.5,fontStyle:'bold'},
      bodyStyles:{fontSize:7},
      columnStyles:{3:{cellWidth:34},4:{cellWidth:34}},
      margin:{left:12,right:12},
  ...
  // Por diagnóstico
  var diagArr=Object.entries(st.diagMap).sort(function(a,b){ return b[1]-a[1]; });
  if(diagArr.length){
    // Check if need new page
    if(y>230){ doc.addPage(); pdfFooter(doc); y=20; }
    doc.setFont(undefined,'bold'); doc.setFontSize(8); doc.setTextColor(15,43,74);
    doc.text('DISTRIBUIÇÃO POR DIAGNÓSTICO',12,y); y+=4; doc.setTextColor(0,0,0);
    var half = Math.ceil(diagArr.length/2);
    var left = diagArr.slice(0,half);
    var right = diagArr.slice(half);
    var maxRows = Math.max(left.length,right.length);
    var tblRows = [];
    for(var i=0;i<maxRows;i++){
      tblRows.push([
        left[i]?left[i][0]:'', left[i]?left[i][1]:'',
        right[i]?right[i][0]:'', right[i]?right[i][1]:''
      ]);
    }
    doc.autoTable({
      startY:y,
      head:[['Diagnóstico','N','Diagnóstico','N']],
      body:tblRows, theme:'grid',
      headStyles:{fillColor:[15,43,74],textColor:255,fontSize:7,fontStyle:'bold'},
      bodyStyles:{fontSize:7.5},
      columnStyles:{1:{halign:'center',cellWidth:12},3:{halign:'center',cellWidth:12}},
      margin:{left:12,right:12}
    });
    y = doc.lastAutoTable.finalY+5;
  }
  ...
  // Por procedimento
  var procArr=Object.entries(st.procMap).sort(function(a,b){ return b[1]-a[1]; });
  if(procArr.length){
    if(y>230){ doc.addPage(); pdfFooter(doc); y=20; }
    doc.setFont(undefined,'bold'); doc.setFontSize(8); doc.setTextColor(15,43,74);
    doc.text('DISTRIBUIÇÃO POR PROCEDIMENTO',12,y); y+=4; doc.setTextColor(0,0,0);
    doc.autoTable({
      startY:y,
      head:[['Procedimento','Nº']],
      body:procArr, theme:'grid',
      headStyles:{fillColor:[15,43,74],textColor:255,fontSize:7.5,fontStyle:'bold'},
      bodyStyles:{fontSize:7.5},
  ...
  pdfFooter(doc);
  doc.save('CirurgiaGeral_ReuniaoMatinal_'+today+'.pdf');
  toast('PDF Reunião gerado');
}
window.exportPDF = exportPDF;

/* ── PDF RESUMO DIÁRIO (compacto, 1 página) ── */
function exportResumoPDF(){
  ...
  var y = pdfHeader(doc,'Cirurgia Geral — Resumo Diário');
  doc.setTextColor(0,0,0);
  ...
  // KPIs grandes numa grelha 4×2
  var kpiDefs = [
    ['Total Intervenções', st.total],
    ['Urgentes', st.urg],
    ['Eletivas / Semiurgentes', st.ele],
    ['Complicações', st.comp],
    ['Óbitos', st.obt],
    ['Doentes ≥ 15 anos', st.mai],
    ['Doentes < 15 anos', st.men],
    ['Taxa de Complicação', st.total?Math.round(st.comp/st.total*100)+'%':'—']
  ];
  var cols = 4;
  var cw = (W-24)/cols;
  var ch = 18;
  var kpiColors = [[15,43,74],[60,80,160],[60,80,160],[180,70,0],[200,30,30],[15,43,74],[15,43,74],[100,60,0]];
  for(var i=0;i<kpiDefs.length;i++){
    var cx = 12+(i%cols)*cw;
    var cy = y+Math.floor(i/cols)*(ch+2);
    var col = kpiColors[i];
    doc.setFillColor(col[0],col[1],col[2]);
    doc.roundedRect(cx,cy,cw-2,ch,2,2,'F');
    doc.setTextColor(255,255,255);
    doc.setFontSize(14); doc.setFont(undefined,'bold');
    doc.text(String(kpiDefs[i][1]),cx+cw/2-1,cy+10,{align:'center'});
    doc.setFontSize(6.5); doc.setFont(undefined,'normal');
    doc.text(kpiDefs[i][0],cx+cw/2-1,cy+15,{align:'center'});
  }
  y += Math.ceil(kpiDefs.length/cols)*(ch+2)+6;
  doc.setTextColor(0,0,0);

  // Demo compacta
  doc.autoTable({
    startY:y,
    head:[['Grupo','H ≥ 15','F ≥ 15','H < 15','F < 15','Total']],
    body:[['Intervenções',st.cats['H≥15'],st.cats['F≥15'],st.cats['H<15'],st.cats['F<15'],st.total]],
    theme:'grid',
    headStyles:{fillColor:[15,43,74],textColor:255,fontSize:7.5,halign:'center'},
    bodyStyles:{fontSize:9,halign:'center'},
  ...
  // Top diagnósticos
  var diagArr=Object.entries(st.diagMap).sort(function(a,b){ return b[1]-a[1]; }).slice(0,8);
  if(diagArr.length){
    doc.setFont(undefined,'bold'); doc.setFontSize(8); doc.setTextColor(15,43,74);
    doc.text('TOP DIAGNÓSTICOS',12,y); y+=4; doc.setTextColor(0,0,0);
    doc.autoTable({
      startY:y,
      head:[['Diagnóstico','N']],
      body:diagArr,
  ...
  pdfFooter(doc);
  doc.save('CirurgiaGeral_Resumo_'+today+'.pdf');
  toast('Resumo Diário gerado');
}
window.exportResumoPDF = exportResumoPDF;

/* ── PDF LISTA DE DOENTES ── */
function exportListaPDF(){
  var jsPDFLib = getJsPDF(); if(!jsPDFLib) return;
  var filtData = document.getElementById('lista-filtro').value;
  var recs = filtData
    ? state.registos.filter(function(r){ return r.data===filtData; })
    : state.registos.slice();
  recs.sort(function(a,b){ return (a.data+a.hora).localeCompare(b.data+b.hora); });
  ...
  var y = pdfHeader(doc,'Cirurgia Geral — Lista de Doentes');
  doc.setTextColor(0,0,0);
  doc.setFontSize(8.5); doc.setFont(undefined,'normal');
  doc.text('Período: '+label+' | Total: '+recs.length+' registo(s)',12,y); y+=6;

  if(!recs.length){
    doc.setFontSize(10); doc.text('Sem registos para o período selecionado.',12,y);
    pdfFooter(doc);
    doc.save('CirurgiaGeral_Lista_'+todayStr()+'.pdf');
    toast('PDF gerado (sem registos)');
    return;
  }
  ...
  var rows = recs.map(function(r,i){
    return [
      ...
      r.diag||'—',
      r.procCir||'—',
      r.tipo==='urgente'?'Urgente':r.tipo==='eletiva'?'Eletiva':'Semi',
      r.anest||'—',
      r.resultado==='ok'?'OK':r.resultado==='complicacao'?'Complicação':'Óbito',
      r.destino||'—',
      r.cirurgiao||'—'
  ...
  doc.autoTable({
    startY:y,
    head:[['#','Data','Hora','Doente','Diagnóstico','Procedimento','Tipo','Anest.','Resultado','Destino','Cirurgião']],
    body:rows, theme:'striped',
    headStyles:{fillColor:[15,43,74],textColor:255,fontSize:6.5,fontStyle:'bold'},
    bodyStyles:{fontSize:6.5},
    columnStyles:{4:{cellWidth:32},5:{cellWidth:32}},
    margin:{left:12,right:12},
    didDrawPage:function(){ pdfFooter(doc); }
  });

  pdfFooter(doc);
  doc.save('CirurgiaGeral_Lista_'+(filtData||todayStr())+'.pdf');
  toast('PDF Lista gerado');
}
window.exportListaPDF = exportListaPDF;
